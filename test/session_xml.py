...
parser.add_argument('--agent', dest='agent', required=True) #our agent
...
for opponent in args.party_args:
    ...
    party = ET.SubElement(parties, 'party', {'classPath': args.agent})
    ET.SubElement(party, 'properties')
    party = ET.SubElement(parties, 'party', {'classPath': opponent})
    ET.SubElement(party, 'properties')
